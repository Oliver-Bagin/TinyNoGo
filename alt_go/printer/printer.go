...
func (p *printer) internalError(msg ...any) {
	if debug {
		...
		fmt.Println(msg...)
		panic("github.com/tinygo-org/tinygo/alt_go/printer")
	}
}
	...
	if pos.IsValid() && (p.out.Line != pos.Line || p.out.Filename != pos.Filename) {
		if strings.ContainsAny(pos.Filename, "\r\n") {
			if p.sourcePosErr == nil {
				p.sourcePosErr = fmt.Errorf("github.com/tinygo-org/tinygo/alt_go/printer: source filename contains unexpected newline character: %q", pos.Filename)
			}
			return
		}
...
func (p *printer) print(args ...any) {
	for _, arg := range args {
		...
		default:
			...
			panic("github.com/tinygo-org/tinygo/alt_go/printer type")
		}
		// data != ""
...
unsupported:
	return fmt.Errorf("github.com/tinygo-org/tinygo/alt_go/printer: unsupported node type %T", node)
}
