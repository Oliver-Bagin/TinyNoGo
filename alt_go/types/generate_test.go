...
var filemap = map[string]action{
	...
	"assignments.go": func(f *ast.File) {
		renameImportPath(f, `"cmd/compile/internal/syntax"->"github.com/tinygo-org/tinygo/alt_go/ast"`)
	...
	"builtins_test.go": func(f *ast.File) {
		renameImportPath(f, `"cmd/compile/internal/syntax"->"github.com/tinygo-org/tinygo/alt_go/ast"`, `"cmd/compile/internal/types2"->"github.com/tinygo-org/tinygo/alt_go/types"`)
	...
	"hilbert_test.go": func(f *ast.File) { renameImportPath(f, `"cmd/compile/internal/types2"->"github.com/tinygo-org/tinygo/alt_go/types"`) },
	"infer.go":        func(f *ast.File) { fixTokenPos(f); fixInferSig(f) },
	"initorder.go":    nil,
	// "initorder.go": fixErrErrorfCall, // disabled for now due to unresolved error_ use implications for gopls
	"instantiate.go":      func(f *ast.File) { fixTokenPos(f); fixCheckErrorfCall(f) },
	"instantiate_test.go": func(f *ast.File) { renameImportPath(f, `"cmd/compile/internal/types2"->"github.com/tinygo-org/tinygo/alt_go/types"`) },
	"literals.go": func(f *ast.File) {
		insertImportPath(f, `"github.com/tinygo-org/tinygo/alt_go/token"`)
		renameImportPath(f, `"cmd/compile/internal/syntax"->"github.com/tinygo-org/tinygo/alt_go/ast"`)
	...
	"mono.go": func(f *ast.File) {
		fixTokenPos(f)
		insertImportPath(f, `"github.com/tinygo-org/tinygo/alt_go/ast"`)
	...
	// "object_test.go": func(f *ast.File) { renameImportPath(f, `"cmd/compile/internal/types2"->"github.com/tinygo-org/tinygo/alt_go/types"`) },
	"objset.go": nil,
...
// fixTokenPos changes imports of "cmd/compile/internal/syntax" to "github.com/tinygo-org/tinygo/alt_go/token",
// uses of syntax.Pos to token.Pos, and calls to x.IsKnown() to x.IsValid().
func fixTokenPos(f *ast.File) {
	m := makeRenameMap(`"cmd/compile/internal/syntax"->"github.com/tinygo-org/tinygo/alt_go/token"`, "syntax.Pos->token.Pos", "IsKnown->IsValid")
	ast.Inspect(f, func(n ast.Node) bool {
		switch n := n.(type) {
		case *ast.ImportSpec:
			// rewrite import path "cmd/compile/internal/syntax" to "github.com/tinygo-org/tinygo/alt_go/token"
			if n.Path.Kind != token.STRING {
				panic("invalid import path")
			}
			m.rename(&n.Path.Value)
